...
export function TopList({ title, items }: { title: string, items: { name: string, total: string }[] }) {
	...
	return (
		<div className="rounded-2xl p-5 h-full flex flex-col" style={{ border: '1px solid var(--n-border)', background: 'var(--n-bg)', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
			<h3 className="text-[12px] font-semibold text-gray-800 mb-6 tracking-wide">{title}</h3>
			<div className="flex-1 flex flex-col justify-center gap-4">
				{items.length === 0 && (
					<div className="text-[12px] italic text-gray-400">No data available.</div>
				)}
				{items.map((item, i) => {
					...
					return (
						<div key={i} className="relative group">
							<div className="flex justify-between items-baseline text-[12px] mb-2 z-10 relative">
